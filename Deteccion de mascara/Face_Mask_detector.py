...
while True:
    ...
    print("[INFO] Detectando mascarilla...")
    net.setInput(blob)
    ...
    for i in range(0 , detections.shape[2]):
        ...
        if confidence > 0.5:
            ...
            label = "Mascarilla detectada" if mask > withoutMask else "Mascarilla NO detectada"
            color = (0,255,0) if label == "Mascarilla detectada" else (0,0,255)
            if label == "Mascarilla detectada":
                ser.write(uno)
            elif label == "Mascarilla NO detectada":
                ser.write(cero)
